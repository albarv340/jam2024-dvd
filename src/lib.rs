...
pub fn get_canvas(canvas_id: &str) -> web_sys::HtmlCanvasElement {
    let document = web_sys::window().unwrap().document().unwrap();
    let canvas = document.get_element_by_id(canvas_id).unwrap();
    return canvas.dyn_into::<web_sys::HtmlCanvasElement>().expect("REASON");
}

pub fn init_webgl_context(canvas_id: &str) -> Result<WebGlRenderingContext, JsValue> {
    let canvas: web_sys::HtmlCanvasElement = get_canvas(canvas_id);
...
pub fn draw_rectangle(
    ...
    let canvas_width: f32 = canvas.width() as f32;
    let canvas_height: f32 = canvas.height() as f32;
    let size: f32 = size.unwrap_or(200.0) / canvas_height;

    let x_ratio: f32 = canvas_width / canvas_height;
    let vert_x: f32 = (x.unwrap_or(0.0) / canvas_width - 0.5) * 2.0;
    let vert_y: f32 = (-y.unwrap_or(0.0) / canvas_height + 0.5) * 2.0;

    
    let vertices: [f32; 18] = [
        vert_x + size / x_ratio, vert_y + size, 0.0, // top right # first triangle
        vert_x + -size / x_ratio, vert_y + -size, 0.0, // bottom left # first triangle
        vert_x + size / x_ratio, vert_y + -size, 0.0, // bottom right # first triangle
        vert_x + -size / x_ratio, vert_y + -size, 0.0, // bottom left # second triangle
        vert_x + size / x_ratio, vert_y + size, 0.0, // top right # second triangle
        vert_x + -size / x_ratio, vert_y + size, 0.0, // top left # second triangle
    ];
    setup_vertices(&gl, &vertices, &shader_program);
